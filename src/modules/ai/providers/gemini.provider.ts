...
export class GeminiProvider implements IAiProvider {
  ...
  async text(input: TTextInput): Promise<TAiResponse> {
    const model = input.model ?? envConfig.geminiTextModel ?? 'gemini-3-flash-preview';
  ...
  async understandImage(input: TImageUnderstandInput): Promise<TAiResponse> {
    const model = input.model ?? envConfig.geminiVisionModel ?? 'gemini-3-flash-preview';
  ...
  async generateImage(input: TImageGenerateInput): Promise<TAiResponse> {
    const model = input.model ?? envConfig.geminiVisionModel ?? 'gemini-2.5-flash-image';
